...
/*   Updated: 2023/09/06 16:10:10 by isbraz-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_first_and_last_line(t_game map)
{
	...
	while (map.map[map.nlines - 2] && map.map[map.nlines - 2][i] == '1')
		i++;
	if (j == (ft_strlen(map.map[0]) - 2) && (i == ft_strlen(map.map[map.nlines - 2])))
		return (1);
	return (0);
}
...
static int	ft_check_content(t_game map)
{
	...
	while (map.map[i] && map.map[i] != NULL && map.map[i][++j] != '\0')
	{
		...
		&& map.map[i][j] != 'C' && map.map[i][j] != 'E' && map.map[i][j] != '\n')
			return (0);
		if (map.map[i][j] == 'P')
			map.p++;
		if (map.map[i][j] == 'C')
...
static int	ft_is_line_equal(t_game map)
{
	...
	while (ft_strlen(map.map[i]) == j)
		i++;
	if (map.map[i] == NULL || ft_strlen(map.map[0]) == ft_strlen(map.map[i]) + 2)
		return (1);
	return (0);
}
